...
select first_name, house_type from houses h join owners o on h.owner_id = o.id where length(first_name) = (select max(length(first_name))
                                                                                                           from owners
) and house_type ='House';
-- - 2. Уйлордун баалары 1500, 2000 дин аралыгында бар болсо true чыгар, жок болсо false чыгар.
select true from houses order by price between 1500 and 2000;
-- - 3. id_лери 5, 6, 7, 8, 9, 10 го барабар болгон адресстерди жана ал адрессте кайсы уйлор бар экенин чыгар.
select a,houses from houses join addresses a on a.id = houses.address_id where address_id in (5,6,7,8,9,10);
-- - 4. Бардык уйлорду, уйдун ээсинин атын, клиенттин атын, агенттин атын чыгар.
select houses,o.first_name,c.first_name,a.name from houses inner join owners o on o.id = houses.owner_id inner join rent_info ri on houses.id = ri.house_id inner join customers c on c.id = ri.customer_id inner join agencies a on a.id = ri.agency_id;
-- - 5. Клиенттердин 10-катарынан баштап 1999-жылдан кийин туулган 15 клиентти чыгар
select * from customers where date_of_birth > '1/1/1999' offset 10 limit 15;--(2 эле клиент бар экен)
-- - 6. Рейтинги боюнча уйлорду сорттоп, уйлордун тайптарын, рейтингин жана уйдун ээлерин чыгар. (asc and desc)
select houses.rating,house_type,o.first_name from houses inner join owners o on o.id = houses.owner_id order by rating asc;
-- - 7. Уйлордун арасынан квартиралардын (apartment) санын жана алардын баасынын суммасын чыгар.
select house_type,sum(price) from houses where house_type ='Apartment' group by house_type;
...
select a2,ag,h2,a2 from agencies ag inner join addresses a2 on a2.id = ag.address_id inner join rent_info ri on ag.id = ri.agency_id inner join houses h2 on ag.address_id = h2.address_id;
...
select * from houses full  join rent_info ri on houses.id = ri.house_id full join customers c on c.id = ri.customer_id;
